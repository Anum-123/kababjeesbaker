...
<body>
  ...
  <div class="bottom-container1">
    <div class="bottom1">
      <h1 style=" color: white; margin-left: 40px;margin-top: 5px; ">Registration</h1>
      <form action="" method="POST" onsubmit="return validateForm()" name="registration_form">

        <input style="margin-top: 30px;" name="firstname" type="text" placeholder="firstName" required class="input-field" /><br><br>

          <input style="margin-top: 30px;"  type="text"  name="lastName" placeholder="lastName" required class="input-field" /><br><br>
       
          <input style="margin-top: 30px;"  type="text" name="address" placeholder="address" required class="input-field" /><br><br><br>

          <input type="tel" name="phone"  placeholder="Number" pattern="[0-9]{12,13}" title="Please enter a valid phone number (12-13 digits)" required class="input-field" /><br><br>
       
          <input style="margin-top: 30px;"  type="text" name="email" placeholder="email" required class="input-field" /><br><br>

        <input style="margin-top: 30px;" type="password" placeholder="password" name="password"  required class="input-field" /><br><br>
    
        
        <input style="background-color: red; color: white; width: 40%; 
            height: 30px; margin-left: 10px;margin-top: 30px; border: 2px red;" type="submit" name="savebtn" value="registration"><br><br>
    </div>
    </form>
    <?php
    if (isset($_POST['savebtn'])) {
        ...
        $checkQuery = "SELECT * FROM registration_form WHERE email='$email'";
    ...
    if (mysqli_num_rows($checkResult) > 0) {
        ...
        $query = "INSERT INTO registration_form (firstname,lastname,address,phone,email,password) VALUES('$fname','$lname','$address','$phone','$email', '$password')";
        $data = mysqli_query($con, $query);

        if ($data) {
          echo "<script>alert('Registration successful! Click OK to proceed to login.');
          window.location.href='http://localhost/anumprogram/web/login.php';
          </script>";
        } else {
